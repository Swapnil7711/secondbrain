...
export default function AddTextSource({ hideAddSourceOptions, customer, botInfo }) {
    ...
    async function handleSubmit(event) {
        ...
        // check if bulkText is empty
        if (bulkText === '') {
            showSnackbar('Please enter some text to index.');
            return;
        }
        setindexing(true);
        ...
        if (!success) {
            ...
            showSnackbar(`Indexing failed for ${source}.`);
            setindexing(false);
        }
        for (let index = 0; index < chunks.length; index++) {
            await generateEmbeddings(chunks[index]).then(async (embedding) => {
                if (embedding) {
                    await addInfoToSupabase(source, title, chunks[index], embedding, source_id);
                } else {
