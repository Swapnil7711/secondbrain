...
export default function AddURLListSource({ hideAddSourceOptions, customer, botInfo }) {
    ...
    async function handleSubmit(event) {
        ...
        for (const url of urls) {
            ...
            console.log(mdLocal);
            // if mdLocal is null then continue and skip this url 
            if (!mdLocal) {
            ...
            var { success, source_id } = await addSourceToSupbase(url);
            if (!success) {
    ...
    const addSourceToSupbase = async (url): Promise<any> => {
        try {
    ...
    const addInfoToSupabase = async (source: string, title: string, markdown: string, embedding: number[], source_id): Promise<void> => {
        try {
            // Insert the new row into the table
            console.log(`source: ${source} title: ${title} markdown: ${markdown} embedding: ${embedding} source_id: ${source_id}`);
    ...
    const scrapeWebsite = async (url: string): Promise<string | null> => {
        try {
            const response = await fetch(`/api/scrape-web?url=${encodeURIComponent(url)}`);
            if (!response.ok) {
    ...
    const processTextareaInput = (textareaText) => {
        const lines = textareaText.split('\n');
        const urls = lines.flatMap(line => line.split(' ').map(url => url.trim()));
        const validUrlsSet = new Set(urls.filter(url => url !== '' && isValidUrl(url)));
        return Array.from(validUrlsSet);
    };
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        ...
                        <div className="relative mt-1 rounded-md shadow-sm">
                            <textarea
                                name="url"
                                onChange={(e) => seturls(processTextareaInput(e.target.value))}
                                id="url"
